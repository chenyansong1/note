...
# 跳转到上一个函数 or 跳转到下一个函数(可以用来向上或者向下跳转几行)
ctrl+[
ctrl+]
...
2.Vim普通模式下，c 命令是一个非常实用的命令，如 :
# cc 可用来改写当前行(即删除当前行并进入插入模式)，
# c$ 改写到行末，
# ci" 改写双引号中的内容(双引号可以换成其他配对的符号)，c2w 改写当前光标下两个单词，ct( 改写到小括号前。(c 命令与 i、a、w 等组合可组成非常多实用的命令)
```
...
1.  选定文本块。使用v进入可视模式，移动光标键选定内容。**接着进行如下的操作**
