...
![](http://ols7leonh.bkt.clouddn.com//assert/img/bigdata/hadoop/shell/shell_copy2hdfs.png)
...
do
	...
	# if [ "access.log" = "$fileName" ];then
		...
		#将待上传的文件path写入一个列表文件willDoing，有日期的（如：$log_toupload_dir/willDoing.20161129, $log_toupload_dir/willDoing.20161221, ... ）
		echo $log_toupload_dir"xxxxx_click_log_$fileName"$date >> $log_toupload_dir"willDoing."$date
		
	fi
	
done
#找到列表文件willDoing($log_toupload_dir/willDoing.20161129, $log_toupload_dir/willDoing.20161221, )
ls $log_toupload_dir | grep will |grep -v "_COPY_" | grep -v "_DONE_" | while read line		#	line = $log_toupload_dir/willDoing.20161129
do
	#打印信息
