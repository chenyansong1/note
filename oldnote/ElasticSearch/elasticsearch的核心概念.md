...
![](/assert/img/ES/near_realtime.png)
...
![](/assert/img/ES/shard_replica.png)


1.index会被拆分为多个shard,每个shard就会存放这个index的一部分数据,这些shard会散落在多台服务器上面
2.shard其实叫primary shard,一般简称为shard,replica其实叫replica shard,一般简称为replica
3.高可用,一个shard宕机,数据不丢,服务器继续提供
4.有replica的存在提升了搜索请求的吞吐量(因为replica的存在提高了读性能)
