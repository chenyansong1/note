...
class JobGenerator(jobScheduler: JobScheduler) extends Logging {
  ...
  /*
  只要JobGenerator,这里会初始化一个开始时间,
  后面,根据我们自己设置的batch interval,每到一个batch interval
  ...
  // 定时调度generateJobs()方法
  // 传入一个time时间,其实就是batch interval内的时间段
  private def generateJobs(time: Time) {
    ...
    Try {
      // 拿到ReceiverTracker,调用其allocateBlocksToBatch()方法,
      // 将当前时间段内的block,分配给一个batch
      // 并为其创建一个RDD
      jobScheduler.receiverTracker.allocateBlocksToBatch(time) // allocate received blocks to batch
      // 然后调用DStreamGraph的generateJobs()方法,
      // 根据定义的DStream之间的依赖关系和算子生成job
