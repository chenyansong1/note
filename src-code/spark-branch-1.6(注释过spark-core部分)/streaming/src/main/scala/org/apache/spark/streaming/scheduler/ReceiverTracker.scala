...
class ReceiverTracker(ssc: StreamingContext, skipReceiverLaunch: Boolean = false) extends Logging {
  ...
  def start(): Unit = synchronized {
    ...
    if (!receiverInputStreams.isEmpty) {
      // 拿到启动Receiver的rpc,后面会用这个rpc去远程启动Receiver
  ...
  private def launchReceivers(): Unit = {
    // receiverInputStreams是一个Array,里面有所有创建的DStream
    // 这里是遍历每个DStream,调用其getReceiver()方法,得到receiver
    // 最后返回的是所有DStream的receiver
  ...
  private class ReceiverTrackerEndpoint(override val rpcEnv: RpcEnv) extends ThreadSafeRpcEndpoint {
    ...
    private def startReceiver(
      ...
      // 在Worker节点上启动Receiver函数
      // 这个函数的执行,是在Executor上执行的,即Receiver的启动在Executor上
      ...
      // 真正的启动,将启动Receiver的函数(startReceiverFunc)分布到
      // 各个Worker节点的Executor上去执行
