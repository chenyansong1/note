 ...
 */
/*
在创建完成StreamingContext的初始化之后,创建了DStreamGraph,JobScheduler等关键组件
之后就会调用streamContext的sockeTextStream等方法来创建输入DStream
然后针对输入DStream执行一系列的Transformation转换操作
最后,会执行一个output输出操作,来触发针对一个一个的batch的job的触发和执行

最后会通过StreamingContext.start()方法来启动Spark Streaming的应用程序
start()方法其实就是创建StreamingContext的相关的另外两个重要的组件:
ReceiverTracker, JobGenerator
然后启动整个spark streaming应用程序的输入DStream对应的Receiver

Receiver是在spark集群的某个Worker节点上的Executor中启动的
 */
class StreamingContext private[streaming] (
  ...
  /*
  这里就保存了,我们定义的spark Streaming Application中一系列的DStream的依赖关系
  以及互相之间的算子的应用
   */
  ...
  /*
  涉及到job的调度
  JobGenerator会负责每隔batch interval 的时间间隔,生成一个job,
  然后通过JobScheduler来调度和提交job
  底层其实还是基于spark core的计算引擎(DAGScheduler,TaskScheduler,Worker,Executor,Task)
  如果你定义了reduceByKey这种算子,还是有shuffle
