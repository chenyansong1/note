...
private[streaming] class ReceiverSupervisorImpl(
  ...
  private val receivedBlockHandler: ReceivedBlockHandler = {
    // 如果开启了预写日志机制,"spark.streaming.receiver.writeAheadLog.enable"默认为false
    // 如果为true,那么receivedBlockHandler的类型就是WriteAheadLogBasedBlockHandler
    // 如果为false,那么receivedBlockHandler的类型就是BlockManagerBasedBlockHandler
  ...
  def pushAndReportBlock(
    ...
    // 存储block到BlockManager中
    val blockStoreResult = receivedBlockHandler.storeBlock(blockId, receivedBlock)
    logDebug(s"Pushed block $blockId in ${(System.currentTimeMillis - time)} ms")
    val numRecords = blockStoreResult.numRecords
    // block的元数据信息封装
    val blockInfo = ReceivedBlockInfo(streamId, numRecords, metadataOption, blockStoreResult)
    // 发送AddBlock到Driver的ReceiverTracker
  ...
  override protected def onStart() {
    // 这里是BlockGenerator的ArrayBuffer集合
    // BlockGenerator是Worker的Executor段负责数据接收后的一些存取工作
    // 以及配合ReceiverTracker
