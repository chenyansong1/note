...
private[sql] abstract class AbstractSparkSQLParser
  ...
  def parse(input: String): LogicalPlan = synchronized {
    ...
    initLexical
    // 用lexical.Scanner针对sql语句进行语法检查,分析
    // 满足语法检查结果的话,就使用sql解析器,对sql进行解析,包括词法解析(将sql语句解析成一个一个的短语(token)),语法解析
    // 最后生成一个LogicalPlan
    // 该LogicalPlan仅仅针对sql语句本身生成的纯语法
    // 不涉及任何关联的数据源等信息
    phrase(start)(new lexical.Scanner(input)) match {
        // 其实词法解析就是对select,where,from生成Tree的一个一个的节点,最后生成一个语法树
