...
class SQLContext private[sql](
  ...
  /*
  在触发sql语句执行的时候,会执行executeSql()方法,该方法返回QueryExecution
  这个QueryExecution会触发整个的后续流程
   */
  ...
  def sql(sqlText: String): DataFrame = {
    // spark sql是有lazy特性的,
    // 其实在使用sqlContext.sql("xx")去执行一条sql语句的时候,默认只会调用
    // SqlParser组件(在1.6中是:ddlParser)针对sql语句生成一个:LogicalPlan
    // 然后将LogicalPlan和SQLContext自身封装成DataFrame返回

    // 在用户拿到LogicalPlan和SQLContext封装的DataFrame之后
    // 可以对DataFrame执行一些show,select().show, groupBy().show等操作
    // 或者将DataFrame转成rdd,执行一系列Transformation操作,最后执行一个action操作
    // 在执行了DataFrame的上述要求返回结果数据的操作之后,才会实际去触发spark sql后续的sql执行流程
    // 包括Analyzer,Optimizer,PhysicalPlan

    // parseSql方法返回的是LogicalPlan,LogicalPlan封装了一颗语法树,
    // 这里的LogicalPlan还不知道自己执行的时候,那些数据从哪里来
    // 所以,这里的LogicalPlan叫做Unresolved LogicalPlan
