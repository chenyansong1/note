 ...
 */
/*
对一个单独的TaskSet的任务进行调度,这个类负责追踪每一个task,如果task的任务失败的话,
会负责重试task,知道超过重试的次数限制,并且会通过延迟调整,为这个TaskSet处理本地化调度机制,
他的主要接口时resourceOffer,在这个接口中,TaskSet会希望在一个节点上运行一个任务,
并且接受任务的状态改变消息,来知道他负责的task的状态改变了
 */
private[spark] class TaskSetManager(
  ...
  /*
  这个方法的大致意思是:
  回去判断这个executor在这个本地化级别,之前的等待时间是多少,
