...
private[spark] class MemoryStore(blockManager: BlockManager, memoryManager: MemoryManager)
  ...
  private def tryToPut(
    ...
    // 多线程并发同步
    // 使用多线程的原因是:可能你刚判定内存足够你放,但是其他线程就放入了数据
    ...
    memoryManager.synchronized {
      ...
      // 将数据写入内存的时候,首先调用acquireStorageMemory(),判断内存足够放入数据
      if (enoughMemory) {
        ...
        // dropFromMemory(),此时会将部分内存数据写入到磁盘中
        // 如果持久化级别不支持磁盘,那么从内存移除的数据将丢失
