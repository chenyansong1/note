...
class DAGScheduler(
  ...
  /*
  获取某个stage的父stage
  这个方法的描述:如果从他的最后一个rdd的所有依赖都是窄依赖,那么不会创建任何新的
  stage,然后从后面的rdd往前找父rdd,如果发现了某个rdd是宽依赖的话
  那么就创建一个新的stage(这个stage包含此时的rdd),将这个stage加入到missing中
  所以方法名 getMissingParentStages 返回的是一个stage的父stage很恰当
   */
  private def getMissingParentStages(stage: Stage): List[Stage] = {
    ...
    def visit(rdd: RDD[_]) {
      if (!visited(rdd)) {
        ...
        if (rddHasUncachedPartitions) {
          ...
          底层对应了三个RDD:MapPartitionsRDD,ShuffledRDD,MapPartitionsRDD
          其中ShuffleRDD和MapPartitionsRDD将被划分为一个stage中
           */
          for (dep <- rdd.dependencies) {
            dep match {
                // 如果是宽依赖的话
              case shufDep: ShuffleDependency[_, _, _] =>
                // 那么使用宽依赖的那个rdd创建一个ShuffleMapStage,  mapOutputTracker.registerShuffle
                val mapStage = getShuffleMapStage(shufDep, stage.firstJobId)
                if (!mapStage.isAvailable) {
                  // 有shuffle的rdd(宽依赖)会创建一个新的stage,然后将这个stage加入到missing中
                  missing += mapStage
                }
                // 如果是窄依赖,那么将依赖的rdd(父rdd)放入栈中,该方法调用完成,因为栈中元素不为空,又会进行while循环
    ...
    // 首先向栈中push了最后一个stage的rdd
    waitingForVisit.push(stage.rdd)
    // 然后进行while循环
    while (waitingForVisit.nonEmpty) {
      // 调用自己内部定义的visit方法(从栈中取出rdd,传给方法)
      visit(waitingForVisit.pop())//出栈
    }
    missing.toList
  }
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    //加入到事件监听器,这样job的提交过程在webUI可以实时展现
    listenerBus.post(
      SparkListenerJobStart(job.jobId, jobSubmissionTime, stageInfos, properties))

    // 提交finalStage
    // 这个方法的调用,其实只会导致第一个stag的提交
    // 并且导致其他所有的stage,都会放入waitingStages中
    submitStage(finalStage)

    /*
    stage划分算法很重要:
    知道自己编写的spark Application被划分了几个job
    每个job被划分成了几个stage
    每个stage,包括了你的哪些代码
    在线上,如果你发现某个stage执行的特别慢,或者某个stage一直报错
    你才能针对那个stage对应的代码,去排查问题或者性能调优
  ...
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing.isEmpty) {//当前stage没有父stage
          logInfo("Submitting " + stage + " (" + stage.rdd + "), which has no missing parents")
          // 这里只是提交的是第一个stage,其余的stage将通过递归放入waitingStages
          submitMissingTasks(stage, jobId.get)
        } else {//如果存在父stage,那么会递归调用,直到没有父stage,那么会调用submitMissingTasks方法
          for (parent <- missing) {
            submitStage(parent)//递归调用
          }
          //将当前stage放入waitingStages等待执行的stage的队列中
          waitingStages += stage
        }
      }
    } else {
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    val tasks: Seq[Task[_]] = try {
      stage match {
        case stage: ShuffleMapStage =>
          // 遍历每一个分区,给每个partition创建一个task
          partitionsToCompute.map { id =>
            // 给每个task计算最佳位置
            // 对于finalStage之外的stage,会创建ShuffleMapTask
    ...
    if (tasks.size > 0) {
      ...
      // 最后,针对stage的task,创建TaskSet对象,调用TaskScheduler的submitTasks去提交TaskSet
      // 默认情况下,TaskScheduler实现为TaskSchedulerImpl
  ...
  即:从stage的最后一个rdd开始,去找,哪个rdd的partition是被checkpoint或者cache了
  那么task的最佳位置就是缓存(cache)或者checkpoint的位置
  因为这样的话,task就在那个节点上执行不需要计算之前的rdd了
   */
  private def getPreferredLocsInternal(
    ...
    // 那么task的最佳位置(preferredLocs),就是Nil,那么就要通过TaskScheduler去决定了
    Nil
  }
