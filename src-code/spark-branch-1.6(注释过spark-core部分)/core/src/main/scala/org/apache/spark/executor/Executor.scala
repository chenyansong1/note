...
private[spark] class Executor(
  ...
  def launchTask(
    ...
    // 对于每一个task,都会创建一个TaskRunner(继承了java多线程中的Runnable接口)
    val tr = new TaskRunner(context, taskId = taskId, attemptNumber = attemptNumber, taskName,
      serializedTask)
    // 将TaskRunner放入内存缓存
    runningTasks.put(taskId, tr)
    // executor内部的线程池,这里直接将task放入线程池中执行,
    // 线程池是自动实现了排队机制的,也就是说,如果线程池中的线程没有空闲的,
    // 那么丢进来的线程都是要排队的
  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      try {
        ...
        // 最后,通过正式的反序列化操作,将整个task的数据反序列化回来
        ...
        // 这里的value,对于ShuffleMapTask来说,其实就是MapStatus(可以到到ShuffleMapTask中看方法的返回值就知道)
        // 封装了ShuffleMapTask计算的数据,输出的位置
        // 如果后面还是一个ShuffleMapTask计算的数据呢?
        // 那么就会去联系一个MapOutputTracker,来获取上一个ShuffleMapTask的输出位置,然后通过网络拉取数据
        // ResultTask也是一样
        ...
        // 包括:运行了多长时间,反序列化耗费了多长时间,java虚拟机gc耗费了多长时间,结果的序列化耗费了多长时间
        // 这些东西在SparkUI上显示,
        for (m <- task.metrics) {
        ...
        //重要:其实就是调用了Executor所在的CoarseGrainedExecutorBackend的statusUpdate方法
