...
private[spark] abstract class Task[T](
  ...
  : (T, AccumulatorUpdates) = {
    //创建TaskContext,就是task的执行上下文,记录了task执行的一些全局性的数据
    // 比如说:task重试了几次,包括:task属于哪个stage,task要处理的rdd的那个partition等等
  ...
  //调用到了抽象的方法,那么这个类是抽象的父类,我们需要看他的实现类,我们需要看实现的子类:ShuffleMapTask,ResultTask
