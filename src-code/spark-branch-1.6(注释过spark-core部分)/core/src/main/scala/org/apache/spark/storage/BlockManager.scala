 ...
 */
/*
BlockManager运行在每个节点上,包括Driver和executor都会有一份,这要提供了从本地或者是远程存取数据的功能
...
private[spark] class BlockManager(
  ...
  // 每个BlockManager自己会维护一个map, 是blockId到BlockInfo的映射
  // BlockInfo的作用就是用于作为多线程并发访问同一个block的同步监视器(锁)
  ...
  def initialize(appId: String): Unit = {
    // 首先初始化用于远程block数据传输的BlockTransferService
    blockTransferService.init(this)
    shuffleClient.init(appId)

    // 为当前这个BlockManager,创建一个唯一的BlockManagerId
    // 每个BlockManager都关联一个Executor,BlockTransferService的hostname,以及BlockTransferService的port
    // 所以从BlockManagerId的初始化即可看出,一个BlockManager是通过一个节点上的Executor来唯一标识的
    ...
    // 向BlockManagerMaster进行注册,发送RegisterBlockManager消息
  ...
  private def doGetLocal(blockId: BlockId, asBlockResult: Boolean): Option[Any] = {
    // 首先尝试获取block对应的BlockInfo,以这个BlockInfo作为锁
    val info = blockInfo.get(blockId).orNull
    if (info != null) {
      // 对所有的BlockInfo,都会进行多线程并发访问的同步操作
      // 所以BlockInfo,就相当于是对一个block用于作为多线程并发访问的同步监视器
      info.synchronized {
        ...
        // 如果其他的线程在操作这个block,那么其实会直接会等待获取BlockInfo的排他锁
        ...
        // 如果持久化级别使用了内存,那么就会尝试从MemoryStore中获取数据
        if (level.useMemory) {
        ...
        if (level.useDisk) {
          ...
          if (!level.useMemory) {
            ...
            if (!level.deserialized || !asBlockResult) {
              ...
              // 那么从DISK读取出来数据之后,其实会尝试将其放入MemoryStore中,
  ...
  private def doGetRemote(blockId: BlockId, asBlockResult: Boolean): Option[Any] = {
    ...
    // 首先从BlockManagerMaster上获取每一个blockId对应的BlockManager的信息(后缀_2)
    // 然后随机打乱
    ...
    for (loc <- locations) {
      ...
      val data = try {
        // 连接的时候,使用的是BlockManager的唯一标识,就是:host,port,executorId
  ...
  private def doPut(
    ...
    putBlockInfo.synchronized {
      ...
      try {
        ...
        // 首先根据持久化级别,选择一种BlockStore:MemoryStore,DiskStore,Tachyon
        ...
        // 根据你选择的BlockStore级别,然后根据数据的类型,将数据放入store中
        ...
        if (putBlockStatus.storageLevel != StorageLevel.NONE) {
          ...
          if (tellMaster) {
            // 调用reportBlockStatus()将新写入的block元数据信息发送到BlockManagerMaster
    ...
    // 如果我们的持久化级别,是定义了_2的这种后缀,
    // 说明需要对block进行replication,然后将数据传输到其他节点上
