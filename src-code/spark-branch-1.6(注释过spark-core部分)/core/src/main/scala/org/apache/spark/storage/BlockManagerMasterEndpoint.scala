...
class BlockManagerMasterEndpoint(
  ...
  // 映射了每个executorId到BlockManagerId
  ...
  private def removeBlockManager(blockManagerId: BlockManagerId) {
    // 根据blockManagerId对应的BlockManagerInfo
    val info = blockManagerInfo(blockManagerId)

    // Remove the block manager from blockManagerIdByExecutor.
    // 从blockManagerIdByExecutor中移除executorId对应的blockManagerId
    blockManagerIdByExecutor -= blockManagerId.executorId

    // Remove it from blockManagerInfo and remove all the blocks.
    // 从blockManagerInfo移除blockManagerId对应的BlockManagerInfo
    blockManagerInfo.remove(blockManagerId)

    val iterator = info.blocks.keySet.iterator //def blocks: JHashMap[BlockId, BlockStatus]
    // 遍历/BlockManagerInfo内部所有的block块对应的blockId
  ...
  private def removeExecutor(execId: String) {
    ...
    // 获取executorId对应的BlockManagerId,对其调用removeBlockManager
  ...
  private def register(id: BlockManagerId, maxMemSize: Long, slaveEndpoint: RpcEndpointRef) {
    ...
    // 首先判断,如果本地HashMap种没有指定的BlockManagerId,说明没有注册过,那么此时就会进入,进行注册
    if (!blockManagerInfo.contains(id)) {
      // 根据BlockManager对应的executorId找到对应的BlockManagerId
      // 这里其实是做一个安全的判断,因为如果blockManagerInfo map里,没有BlockManagerId(也就是前面的if条件通过了)
      ...
      // 为BlockManagerId创建一份BlockManagerInfo,并加入到blockManagerInfo map中
  ...
  /*
  更新blockInfo,也就是说:每个BlockManager上,如果block发生了变化,那么都要发送updateBlockInfo来请求BlockMangerMaster
  进行BlockInfo的状态更新
   */
  private def updateBlockInfo(
    ...
    // blockManagerInfo(blockManagerId)返回的是BlockInfo,这个方法是更新block的信息
    blockManagerInfo(blockManagerId).updateBlockInfo(
      blockId, storageLevel, memSize, diskSize, externalBlockStoreSize)

    // 每一个block可能会在多个BlockManager上面
    // 因为如果将StorageLevel设置成为带_2的这种,那么就需要将block replication复制一份到其他BlockManager上
    // blockLocations map其实保存了每个blockId对应的BlockManagerId对应的set集合
    // 所以这里会更新blockLocation中的信息,因为使用set存储BlockManagerId,因此自动就去重了
...
private[spark] class BlockManagerInfo(
  ...
  def updateBlockInfo(
    ...
    if (_blocks.containsKey(blockId)) {
      ...
      // 如果storageLevel是使用的内存,那么就给剩余内存数量加上当前内存量
