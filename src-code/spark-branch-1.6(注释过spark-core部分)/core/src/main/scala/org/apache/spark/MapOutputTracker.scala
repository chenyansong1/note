...
private[spark] abstract class MapOutputTracker(conf: SparkConf) extends Logging {
  ...
  def getMapSizesByExecutorId(shuffleId: Int, startPartition: Int, endPartition: Int)
    ...
    // shuffleId可以代表当前这个stage的上一个stage,shuffle是分为两个stage的
    // shuffle write发生在上一个stage中,shuffle read发生在当前的stage中的
    val statuses = getStatuses(shuffleId) // 返回的类型是:Array[MapStatus]
