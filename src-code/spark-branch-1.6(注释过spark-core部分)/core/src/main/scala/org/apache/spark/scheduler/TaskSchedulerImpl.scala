  ...
  extends TaskScheduler with Logging
{
  ...
  override def submitTasks(taskSet: TaskSet) {
    ...
    this.synchronized {
      // 给每一个TaskSets,都会创建一个TaskSetManager
      // TaskSetManager实际上,在后面,会负责他的那个TaskSet的任务执行状态的监视和管理
    ...
    // 在SparkContext原理剖析的时候,创建TaskScheduler的时候,会同时创建一个Backend
  ...
  private def resourceOfferSingleTaskSet(
    ...
    for (i <- 0 until shuffledOffers.size) {
      ...
      // 如果当前executor的CPU数量,至少大于每个task要使用的CPU数量,默认是1
      if (availableCpus(i) >= CPUS_PER_TASK) {
        try {
          // 调用TaskSetManager的resourceOffer方法
          // 去找到,在这个executor上,就用这种本地化级别,
          // TaskSet的哪些task是可以启动的
          // 遍历使用当前本地化级别,可以在该executor上启动的task
          for (task <- taskSet.resourceOffer(execId, host, maxLocality)) {
            // 放入tasks的这个ArrayBuffer中,给指定的executor加上要启动的task
            tasks(i) += task
            val tid = task.taskId
  ...
  def resourceOffers(offers: Seq[WorkerOffer]): Seq[Seq[TaskDescription]] = synchronized {
    ...
    // 首先:将可用的offers(其中是WorkerOffer,代表的是executor的CPU资源)进行shuffle,也就是说,进行打散,从而做到,尽量负载均衡
    val shuffledOffers = Random.shuffle(offers)
    // Build a list of tasks to assign to each worker.
    // 然后:针对WorkerOffer,创建出一堆需要用的东西
    // 比如tasks,他是一个ArrayBuffer,然后其中的每一个元素又是ArrayBuffer
    // 并且每个子ArrayBuffer的数量是固定的,也就是这个executor可用的CPU数量
    // 也就是根据WorkerOffer中CPU数量确定task数量(如:一个WorkerOffer的core是8,那么在new ArrayBuffer(8) ,也就是子元素的task数量为8,即在该executor上启动8个task )
    val tasks = shuffledOffers.map(o => new ArrayBuffer[TaskDescription](o.cores))
    // 返回的是一个数组:数组中的每个元素是每个executor的CPU核数
    val availableCpus = shuffledOffers.map(o => o.cores).toArray

    // 这个很重要,从rootPool中取出了排序的TaskSet
    // 之前讲解TaskScheduler初始化的时候,我们知道,创建TaskSchedulerImpl和SparkDeploySchedulerBackend之后,
    // 执行一个initialize()方法,在这个方法中,其实会创建一个调度池,这里相当于是说,所有提交的taskSet会放入这个调度池
