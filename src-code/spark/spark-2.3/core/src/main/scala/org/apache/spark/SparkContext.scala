...
class SparkContext(config: SparkConf) extends Logging {
  ...
  def deployMode: String = _conf.getOption("spark.submit.deployMode").getOrElse("client")  //默认是client
  ...
  def isLocal: Boolean = Utils.isLocalMaster(_conf)//是否是本地模式：local or local[n]
   ...
   * a result from one partition)
   */
  def runJob[T, U: ClassTag](rdd: RDD[T], func: Iterator[T] => U): Array[U] = {// 只有在action中才会真正调用runJob, 所以transform都是lazy的
    runJob(rdd, func, 0 until rdd.partitions.length)
  }
