...
class DAGScheduler(
   ...
   */
    // Map<shuffle dependency ID,  ShuffleMapStage>
  ...
  private def getOrCreateShuffleMapStage(
    ...
    // shuffleIdToMapStage = Map<shuffle dependency ID,  ShuffleMapStage>
    shuffleIdToMapStage.get(shuffleDep.shuffleId) match {
      case Some(stage) =>
        stage

      case None =>
        // Create stages for all missing ancestor(祖先) shuffle dependencies.
        // getMissingAncestorShuffleDependencies方法是拿到这个Rdd的往前寻找的所有的ShuffleDependence
  ...
  private def createResultStage(
    ...
    // 拿到所有的父stage，这些stage都是ShuffleMapStage
    val parents = getOrCreateParentStages(rdd, jobId)
    val id = nextStageId.getAndIncrement()
    val stage = new ResultStage(id, rdd, func, partitions, parents, jobId, callSite)

    // 加入到dagScheduler的内存缓存中
    stageIdToStage(id) = stage
    updateJobIdStageIdMaps(jobId, stage)
  ...
  private def getOrCreateParentStages(rdd: RDD[_], firstJobId: Int): List[Stage] = {
    // getShuffleDependencies方法返回的一个rdd有shuffle的父依赖
  ...
  /** 找到没有放入shuffleToMapStage 的所有的ShuffleDependency **/
    ...
    while (waitingForVisit.nonEmpty) {
      ...
      if (!visited(toVisit)) {
        ...
        getShuffleDependencies(toVisit).foreach { shuffleDep =>
          if (!shuffleIdToMapStage.contains(shuffleDep.shuffleId)) {//在shuffleIdToMapStage中不存在
            ancestors.push(shuffleDep)
            waitingForVisit.push(shuffleDep.rdd)// 这里就会将一个依赖的上一个rdd放进来，然后再次循环遍历，找到再上一个ShuffleDependency
   ...
   * This function is scheduler-visible for the purpose of unit testing.
    *  这个方法返回的是 一个RDD的上一层的宽依赖
    *  A--(窄依赖1)--->B--(宽依赖1)--->C---（窄依赖2）--->D----（窄依赖3）--->E
    *  如上面的依赖关系，如果调用这个方法返回的就是：(宽依赖1)
   */
  private[scheduler] def getShuffleDependencies(
    ...
    val visited = new HashSet[RDD[_]]
    val waitingForVisit = new ArrayStack[RDD[_]]
    // 首先将最后一个rdd放入到一个Array中
    waitingForVisit.push(rdd)
    while (waitingForVisit.nonEmpty) {
      // 从Array中取出rdd
      val toVisit = waitingForVisit.pop()
      if (!visited(toVisit)) {// 是否在Set中存在，如果不存在就执行if
        visited += toVisit // 将这个Rdd加入已经存在的队列
        toVisit.dependencies.foreach {
          case shuffleDep: ShuffleDependency[_, _, _] =>
            parents += shuffleDep// 如果是shuffle依赖就放入到parents中
          case dependency =>
            waitingForVisit.push(dependency.rdd)// 如果这个依赖没有shuffle，那么取这个依赖的rdd放入Array中，等待循环遍历
        }
      }
    }
    parents
  ...
  private def getMissingParentStages(stage: Stage): List[Stage] = {
    ...
    def visit(rdd: RDD[_]) {
      if (!visited(rdd)) {
        visited += rdd// 如果该RDD还没有被遍历，那么将这个RDD加入到已经遍历的Set中
        ...
        if (rddHasUncachedPartitions) {
          ...
          for (dep <- rdd.dependencies) {
            dep match {
                /*
                其实对于有shuffle的操作，比如：groupByKey，reduceByKey
                其底层是对应了三个RDD的：MapPartitionsRDD, ShuffleRDD,MapPartitionsRDD ,所以就有了：第一个RDD被放入了一个Stage中，而最后两个RDD被放入到另一个Stage中
                 */
              // 如果是宽依赖，就创建ShuffleMapStage
              case shufDep: ShuffleDependency[_, _, _] => // 如果发现ShuffleDependency, 说明遇到新的stage
                val mapStage = getOrCreateShuffleMapStage(shufDep, stage.firstJobId)  // check shuffleToMapStage, 如果该stage已经被创建则直接返回, 否则newStage
                if (!mapStage.isAvailable) {
                  missing += mapStage
                }

              // 如果是窄依赖，那么将当前依赖的RDD放入栈中
    ...
    while (waitingForVisit.nonEmpty) {
      // 取出栈顶的RDD，然后调用visit方法
      visit(waitingForVisit.pop())  //递归遍历，直到找到所有的 shuffleMapStage
    }
    missing.toList
  }
  ...
  private def submitWaitingChildStages(parent: Stage) {
    ...
    // 获取出当前Stage的第一级子Stage
    val childStages = waitingStages.filter(_.parents.contains(parent)).toArray
    // 从waitingStages中移除子Stage
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
      ...
      // 这个finalStage是由finalRDD创建的，所以这是一个属于finalRDD的finalStage
  ...
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        // 拿到的都是需要进行shuffle的stage  | 根据final stage发现是否有parent stage
        // 获取当前Stage的父Stage， 这里很重要
        val missing = getMissingParentStages(stage).sortBy(_.id)

        logDebug("missing: " + missing) // 打印拿到的父Stage

        // 如果当前Stage 没有 父stage, 则直接submit当前stage
        if (missing.isEmpty) {
          ...
          // 如果当前Stage 有 父stage,需要先submit parent, 因为stage之间需要顺序执行
          for (parent <- missing) {
            // 递归调用
            submitStage(parent)
          }
  ...
  /*
  这个方法做了两件事：
  1.提交第一个Stage；为该Stage创建task，task的数量和partition数量相同
  2.提交除了第一个Stage的之外的所有加入到等待队列中的所有的Stage
   */
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    // 计算每个task的最佳位置，对应的是partitionID，到最佳位置的映射
    val taskIdToLocations: Map[Int, Seq[TaskLocation]] = try {
      stage match {
        case s: ShuffleMapStage =>
          // partitionsToCompute中放入的是一个partition的ID，遍历每个分区号，返回 <partitionId, 该分区计算的最佳位置队列>
    ...
    if (tasks.size > 0) {
      ...
      // 调用taskScheduler，将一个Stage产生的tasks，封装成为TaskSet，然后将TaskSet去提交
      ...
      // 按照第一个Stage，在waitingStages中找到第一个Stage的下一个子Stage，然后提交他，这样以此类推，不断的提交Stage
   ...
   * DAGScheduler state accessed by it may require additional synchronization.
    ...
    * 说白了就是从stage的最后一个RDD开始，去寻找哪个RDD的partition是被cache了，或者checkpoint了
    * 那么task的最佳位置 就是 cache（checkpoint）所在的位置， 因为这样的话，task就在那个节点上执行，不需要计算之前依赖的RDD了
   */
  private def getPreferredLocsInternal(
      rdd: RDD[_],// stage 的最后一个RDD
      partition: Int,// 分区号
    ...
    // 1.如果该分区对于当前RDD有缓存，那么直接返回该分区的位置
    val cached = getCacheLocs(rdd)(partition)
    if (cached.nonEmpty) {
      return cached
    }
    // If the RDD has some placement preferences (as is the case for input RDDs), get those
    // 2.寻找当前RDD的partition是否被checkpoint，如果是那么就返回checkpoint的位置
    ...
    // 3.递归调用自己，寻找当前RDD的父RDD是否有cache或者checkpoint，如果有找到，那么返回，如果没有
    ...
    /*
     经过上面的三步，都没有找到
     即：从这个Stage的最后一个RDD到最开始的一个RDD，partition都没有被缓存或者checkpoint，那么task的最佳位置（preferredlocs) 就是Nil，即：没有最佳位置
      */
    Nil
  }
