...
class PairRDDFunctions[K, V](self: RDD[(K, V)])
   ...
   * 转换一种RDD为另一种RDD，只不过他们的value的类型不同
   * Users provide three functions:调用者只需要使用下面的传参
   *
  ...
  def combineByKeyWithClassTag[C](
    ...
    val aggregator = new Aggregator[K, V, C](
      ...
      self.context.clean(mergeCombiners))    //1.Aggregator

    /**RDD本身的partitioner和传入的partitioner相等时, 即不需要重新shuffle, 直接map即可*/
    if (self.partitioner == Some(partitioner)) {
      self.mapPartitions(iter => {    //2. mapPartitions, map端直接调用combineValuesByKey
        val context = TaskContext.get()
        new InterruptibleIterator(context, aggregator.combineValuesByKey(iter, context))
      }, preservesPartitioning = true)
    } else {//如果需要mapSideCombine
      new ShuffledRDD[K, V, C](self, partitioner) //3. ShuffledRDD, 进行shuffle
        .setSerializer(serializer)  //因为需要shuffle，所以需要序列化
        .setAggregator(aggregator)  //Shuffle完后, 在reduce端再做一次combine, 使用
        .setMapSideCombine(mapSideCombine)  //先在partition内部做mapSideCombine
    }
  }
   ...
   * to a "combiner" in MapReduce.
    * mergeValue和mergeCombiners逻辑是相同的, 没有区别
   */
  ...
  def groupByKey(partitioner: Partitioner): RDD[(K, Iterable[V])] = self.withScope {
    ...
    val createCombiner = (v: V) => CompactBuffer(v)   ////创建seq
    val mergeValue = (buf: CompactBuffer[V], v: V) => buf += v  //添加item到seq
    val mergeCombiners = (c1: CompactBuffer[V], c2: CompactBuffer[V]) => c1 ++= c2
    val bufs = combineByKeyWithClassTag[CompactBuffer[V]](
      createCombiner, mergeValue, mergeCombiners, partitioner, mapSideCombine = false)  //不使用map side combine, 因为这样也无法减少传输空间, 所以不需要实现mergeCombiners
