...
class BlockManagerMasterEndpoint(
  ...
  private def removeRdd(rddId: Int): Future[Seq[Int]] = {
    ...
    // 从blockLocations中找出所有该RDD对应的blocks
    val blocks = blockLocations.asScala.keys.flatMap(_.asRDDId).filter(_.rddId == rddId)

    // 从blockManagerInfo和blockLocations中去除这些blocks信息
    ...
    val futures = blockManagerInfo.values.map { bm => // 将RemoveRDD的msg发送给每个slave actors
    ...
    if (locations != null) {
      locations.foreach { blockManagerId: BlockManagerId =>
        ...
        if (blockManager.isDefined) {
          // 移除block从slave的BlockManager
          // Remove the block from the slave's BlockManager.
          // Doesn't actually wait for a confirmation and the message might get lost.
          // If message loss becomes frequent, we should add retry logic here.
          // 这里是向Slave 发送rpc，要求移除block
...
private[spark] class BlockManagerInfo(
    ...
    timeMs: Long,
    val maxOnHeapMem: Long, //BlockManager的memory大小
    val maxOffHeapMem: Long,
    val slaveEndpoint: RpcEndpointRef)  //BlockManager的heartbeat, 会被不停的更新
  extends Logging {
