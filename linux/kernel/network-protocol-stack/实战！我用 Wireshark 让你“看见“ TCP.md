...
转自：https://zhuanlan.zhihu.com/p/142665708
...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-0KVeaUUR-1597503893462)(h[https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/](https://link.zhihu.com/?target=https%3A//cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/)计算机网络/TCP-Wireshark/2.jpg)]
...
![img](https://picb.zhimg.com/80/v2-58f4ee49a0f4d2a792eab82d7d43c900_720w.jpg)

要抓取上面的 ping 命令数据包，首先我们要知道 ping 的数据包是 `icmp` 协议，接着在使用 tcpdump 抓包的时候，就可以指定只抓 icmp 协议的数据包：

![img](https://pic2.zhimg.com/80/v2-667ec4930880224b01a4c2175abf6140_720w.jpg)

那么当 tcpdump 抓取到 icmp 数据包后， 输出格式如下：

![img](https://pic3.zhimg.com/80/v2-7a02ca4e5f1263454a42f5de613f7eec_720w.png)

![img](https://picb.zhimg.com/80/v2-f8ea04a5c3eb06a14815214ecd38904d_720w.jpg)
...
![img](https://pic3.zhimg.com/80/v2-6301b0170d00d27e94c285d3cb49e617_720w.jpg)
...
![img](https://picb.zhimg.com/80/v2-441782fb7bf0018cb20aec33f86c9503_720w.jpg)
...
![img](https://pic1.zhimg.com/80/v2-eafd414366280209d0a17295946b3add_720w.jpg)

接着把 ping.pcap 文件拖到电脑，再用 Wireshark 打开它。打开后，你就可以看到下面这个界面：

![img](https://pic1.zhimg.com/80/v2-6a9f1f0a3e2f359996b02c62f3dece41_720w.jpg)
...
![img](https://pic1.zhimg.com/80/v2-0144010f36f93a5b586a413a6f4192ad_720w.jpg)
...
![img](https://pic3.zhimg.com/80/v2-50f92638e2765c9690a2dbf410c055de_720w.jpg)
...
本次例子，我们将要访问的 [http://192.168.3.200](https://link.zhihu.com/?target=http%3A//192.168.3.200) 服务端。在终端一用 tcpdump 命令抓取数据包：

![img](https://pic4.zhimg.com/80/v2-0b81dc0d351301aae05d049b5a3d1c20_720w.jpg)

接着，在终端二执行下面的 curl 命令：

![img](https://pic2.zhimg.com/80/v2-cf33405d0252d9fcb52a08ab29afed18_720w.jpg)

最后，回到终端一，按下 Ctrl+C 停止 tcpdump，并把得到的 http.pcap 取出到电脑。

使用 Wireshark 打开 http.pcap 后，你就可以在 Wireshark 中，看到如下的界面：

![img](https://pic1.zhimg.com/80/v2-3f01d43764c1743df316926f7c676a1b_720w.jpg)
...
![img](https://pic4.zhimg.com/80/v2-8143aed87f193b4418f2e5beb7eedd85_720w.jpg)
...
![img](https://picb.zhimg.com/80/v2-47b7027fbeb92161cab2d92d7b98fd71_720w.jpg)

取消后，Seq 显示的就是真实值了：

![img](https://pic3.zhimg.com/80/v2-b3cd879ab27d89fa0e689fc6b58f2104_720w.jpg)

可见，客户端和服务端的序列号实际上是不同的，序列号是一个随机值。

这其实跟我们书上看到的 TCP 三次握手和四次挥手很类似，作为对比，你通常看到的 TCP 三次握手和四次挥手的流程，基本是这样的：

![img](https://pic2.zhimg.com/80/v2-3113a329a7caf104da1bfb2319c51172_720w.jpg)
...
![img](https://pic4.zhimg.com/80/v2-7cd732758c50cc72b75dda1ca5ccc3c9_720w.png)
...
![img](https://pic2.zhimg.com/80/v2-d9e7eb761b0b79d3fe2d762c4a8d1a4d_720w.jpg)
...
![img](https://picb.zhimg.com/80/v2-5f72898d7cd624f90b1f455e27d89ce6_720w.jpg)

其间 tcpdump 抓包的命令如下：

![img](https://pic4.zhimg.com/80/v2-a7b58dd239d4e7f15993b91e57a0184f_720w.jpg)

过了一会， curl 返回了超时连接的错误：

![img](https://picb.zhimg.com/80/v2-46bc43e8417c62e3366f544913860909_720w.jpg)

从 `date` 返回的时间，可以发现在超时接近 1 分钟的时间后，curl 返回了错误。

接着，把 tcp_sys_timeout.pcap 文件用 Wireshark 打开分析，显示如下图：

![img](https://pic3.zhimg.com/80/v2-359ae0a21b59f301037b216cd5d630c1_720w.jpg)
...
![img](https://picb.zhimg.com/80/v2-2105bf223630266537da6da7eb6e3cdc_720w.png)
...
![img](https://pic2.zhimg.com/80/v2-461a4536d204a0a7126b40116a8af21d_720w.jpg)

### **实验二：TCP 第二次握手 SYN、ACK 丢包**

为了模拟客户端收不到服务端第二次握手 SYN、ACK 包，我的做法是在客户端加上防火墙限制，直接粗暴的把来自服务端的数据都丢弃，防火墙的配置如下：

![img](https://pic3.zhimg.com/80/v2-7449f95f8d6736db1522709bd67858c4_720w.jpg)

接着，在客户端执行 curl 命令：

![img](https://pic4.zhimg.com/80/v2-7f09a50249b558fef107da25373532c6_720w.jpg)

从 `date` 返回的时间前后，可以算出大概 1 分钟后，curl 报错退出了。

客户端在这其间抓取的数据包，用 Wireshark 打开分析，显示的时序图如下：

![img](https://pic4.zhimg.com/80/v2-147e926ab18d56a165ecb3b0eca1e760_720w.jpg)
...
![img](https://pic3.zhimg.com/80/v2-5649f5290cd4ba6cb325e960c99ec646_720w.jpg)
...
![img](https://pic4.zhimg.com/80/v2-ebe31d4a533ca564191e1b274f3a0012_720w.jpg)
...
![img](https://picb.zhimg.com/80/v2-3f280c647de15b159b40b74f33decc7e_720w.jpg)

接着，在客户端执行如下 tcpdump 命令：

![img](https://pic1.zhimg.com/80/v2-30abb99fb38634174ad914addfadccad_720w.jpg)

然后，客户端向服务端发起 telnet，因为 telnet 命令是会发起 TCP 连接，所以用此命令做测试：

![img](https://pic3.zhimg.com/80/v2-ce6e5e7bc66bb4f9d0f613a071b792bd_720w.jpg)

此时，由于服务端收不到第三次握手的 ACK 包，所以一直处于 `SYN_RECV` 状态：

![img](https://picb.zhimg.com/80/v2-909591333c8dff0b4593819d11f353da_720w.jpg)

而客户端是已完成 TCP 连接建立，处于 `ESTABLISHED` 状态：

![img](https://pic1.zhimg.com/80/v2-88ff77d530ce5fe5608f8b1171321b9c_720w.jpg)

过了 1 分钟后，观察发现服务端的 TCP 连接不见了：

![img](https://picb.zhimg.com/80/v2-a38c33a4b145d0b988a54159dd570def_720w.jpg)
...
![img](https://picb.zhimg.com/80/v2-7ba87e78c78dc16545ba4ae1cbf355b0_720w.jpg)

持续「好长」一段时间，客户端的 telnet 才断开连接：

![img](https://pic2.zhimg.com/80/v2-b707306ece395b84083d253cfd6a4f84_720w.jpg)
...
![img](https://picb.zhimg.com/80/v2-d078b1f339537d7cddc2e7c719050e23_720w.jpg)
...
![img](https://pic3.zhimg.com/80/v2-cda8c0d7f9ec4fb4c57b93556a0dc151_720w.jpg)
...
![img](https://pic1.zhimg.com/80/v2-9445956e611f5f5d0c394b60cf855311_720w.jpg)

在 Linux 3.7 内核版本中，提供了 TCP Fast Open 功能，这个功能可以减少 TCP 连接建立的时延。

![img](https://pic1.zhimg.com/80/v2-5935e2daa938b909f277b0629cf40c72_720w.jpg)
...
![img](https://pic3.zhimg.com/80/v2-66def5e770c2df5584610fa325b47736_720w.jpg)
...
![img](https://pic4.zhimg.com/80/v2-90bbd34cc8d8a141cf00d22c3afd3516_720w.jpg)
...
![img](https://picb.zhimg.com/80/v2-ca8de808205f3864e26dcdd7b97c6ee8_720w.jpg)
...
![img](https://pic1.zhimg.com/80/v2-fe6252a2c23fece61e20fedf814e9b92_720w.jpg)
...
![img](https://pic4.zhimg.com/80/v2-9afcb888bcd316badc7f9a5006d2729d_720w.png)

接着，发送方会**定时发送窗口大小探测报文**，以便及时知道接收方窗口大小的变化。

以下图 Wireshark 分析图作为例子说明：

![img](https://pic1.zhimg.com/80/v2-7600381faf0a0bf21e48a3d24647c8a8_720w.jpg)
...
![img](https://pic4.zhimg.com/80/v2-b20567335dfbf9f936cb88de010f7bc3_720w.jpg)
...
![img](https://pic3.zhimg.com/80/v2-d016122b6cec275fd59c5d5a6f152f4b_720w.jpg)
...
![img](https://pic1.zhimg.com/80/v2-822ca9f7c13770449ba8ecd04062d5f5_720w.jpg)
...
![img](https://pic3.zhimg.com/80/v2-b740c6c16996082e55292a6c5b958b12_720w.jpg)
...
![img](https://pic2.zhimg.com/80/v2-b8204f7346aaaa0ad0fdc651e59d53a0_720w.jpg)

延迟等待的时间是在 Linux 内核中定义的，如下图：

![img](https://pic3.zhimg.com/80/v2-a01357be64c103f846213de9a444ce2a_720w.jpg)

关键就需要 `HZ` 这个数值大小，HZ 是跟系统的时钟频率有关，每个操作系统都不一样，在我的 Linux 系统中 HZ 大小是 `1000`，如下图：

![img](https://pic4.zhimg.com/80/v2-7fee290795631511165ad60227bd8453_720w.jpg)
...
![img](https://pic2.zhimg.com/80/v2-25c996e6aeb332b991dc4fb02a30f5ec_720w.jpg)
...
![img](https://pic1.zhimg.com/80/v2-c25f743961eb974f410ddb4981d24225_720w.jpg)
